...
class Index extends \Magento\Framework\App\Action\Action
{
  ...
  public function execute()
  {
	...
	$base_url = $objectManager->get('Magento\Store\Model\StoreManagerInterface')
        ...
        $product = $this->_productLoader->create()->load($id_prodotto);
    

        $imageHelperFactory = $objectManager->create("\Magento\Catalog\Helper\ImageFactory");
        $image_url = $imageHelperFactory->create()->init($product, 'product_base_image')->getUrl(); //immagine del prodotto
	...
	foreach($utenti_gda as $utente){
		...
		$this->send($email,'', $id_customer, $base_url, $image_url,'',$id_prodotto,'',true, $email_text,$oggetto, $titolo, $riepilogo);	
	...
	exit;
     ...
     public function send($email,$token,$id_customer,$base_url,$url_image,$id_preorder,$id_prodotto, $quantita,$adesione = false, $email_text, $oggetto,$titolo, $riepilogo) {
     ...
     //DATA PRODOTTO
     $prodotto = $this->_productLoader->create()->load(intval($id_prodotto));
     $nome_prodotto = $prodotto->getName();
     $prezzo =  $prodotto->getPrice();
     $short_description = $prodotto->getShortDescription();


		 $body = $this->getBody($url_image,$nome,$cognome,$id_preorder,$nome_prodotto,$prezzo,$short_description,$quantita,$token, $base_url,$adesione,$email_text,$titolo, $riepilogo);
         ...
         return $sent;
		    ...
		    public function getBody($img,$nome,$cognome,$id_preorder,$nome_prodotto,$prezzo,$short_description,$quantita,$token, $base_url,$adesione,$email_text,$titolo,$riepilogo){
	  ...
	  $riep = $riepilogo == "false" ? "display:none" : "";
...
<span style='display:inline'>".$email_text."</span><br><br><br>
                                                                        ...
                                                                        <td style='font-family:verdana,arial!important'>
                                                                           <table class='m_-4013470537037416767pad_null' cellspacing='0' cellpadding='0' width='100%' style='font-family:verdana,arial!important;border-collapse:collapse'>
                                                                              <tbody>
                                                                                    ...
                                                                                    <td style='font-family:verdana,arial!important'>
                                                                                       <table class='m_-4013470537037416767pad_null' cellspacing='0' cellpadding='0' width='100%' style='font-family:verdana,arial!important;border-collapse:collapse'>
                                                                                          <tbody>
                                                                                             ...
                                                                                             <tr style='display:none;border-bottom-width:1px;border-bottom-style:solid;border-bottom-color:rgb(204,204,204)'>
                                                                                 ...
                                                                                 <tr  style='display:none' style='text-align:right'>
